...
The config file can only bind loopback. Use the `-listen` flag to bind another
address, such as `0.0.0.0:8317` inside a container.
...
| `claudex service <command>` | macOS: `start`, `stop`, `restart`, `status`, `models`, or `login [-no-browser]` |
...
Run `claudex -help` for the authoritative list.

| Flag | Behavior |
| --- | --- |
| `-config PATH` | Use another config file |
| `-init` | Create the initial config and client key without overwriting existing files |
| `-login` | Codex sign-in using PKCE |
| `-no-browser` | Print the sign-in URL without opening it |
| `-dashboard` | Enable `/dashboard` and its authenticated metrics API |
| `-listen ADDRESS` | Override the listener. This is the only way to bind a non-loopback address. The gateway then logs a warning, because the client key travels over plaintext HTTP. |
