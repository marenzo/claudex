...
func (s *Status) gateway(cfg config.Config) []Check {
	...
	if err != nil {
		return []Check{{LevelWarn, "gateway", fmt.Sprintf("not answering at %s; start it with claudex or claudex service start", base)}}
	}
...
func (s *Status) service() Check {
	...
	return Check{LevelWarn, "macos service", "installed but not loaded; run claudex service start"}
}
