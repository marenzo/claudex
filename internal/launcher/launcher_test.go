...
func TestInstallWritesEverythingAndStartsService(t *testing.T) {
	...
	wrapper, _ := os.ReadFile(s.Paths.Wrapper("claudex-service"))
	if !strings.Contains(string(wrapper), s.Paths.Binary()+`" service "$@"`) {
		t.Errorf("wrapper:\n%s", wrapper)
	}
...
func TestFailedStartRestoresPreviousFilesAndService(t *testing.T) {
	...
	for _, path := range []string{filepath.Join(s.Paths.Config, "client-key"), s.Paths.Wrapper("claudex-service"), s.Paths.SettingsFile()} {
...
func TestServiceUsageAndStop(t *testing.T) {
	s, launchd := testService(t)
	ctx := context.Background()
	for _, args := range [][]string{{"unknown"}, {"restart", "-no-browser"}, {"login", "--version"}} {
		if err := s.Run(ctx, args); !IsUsage(err) {
			t.Errorf("%v: %v", args, err)
		}
	}
	if err := s.Run(ctx, []string{"--help"}); err != nil {
		t.Error(err)
	}
	if len(launchd.calls) != 0 {
		t.Errorf("usage errors touched launchd: %v", launchd.calls)
	}
	// Stopping must not require a readable config.
	if err := os.MkdirAll(s.Paths.Config, 0o700); err != nil {
		t.Fatal(err)
	}
	...
	launchd.loaded = true
	if err := s.Run(ctx, []string{"stop"}); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(launchd.calls, []string{"bootout"}) {
		t.Errorf("calls %v", launchd.calls)
	}
	if err := s.Run(ctx, []string{"start"}); err == nil || !strings.Contains(err.Error(), "rerun the installer") {
		t.Errorf("unexpected error %v", err)
	}
}
...
func TestLoginRestartsPreviouslyRunningService(t *testing.T) {
	...
	launchd.loaded = true
	err := s.Run(context.Background(), []string{"login", "-no-browser"})
	...
	if err := s.Run(context.Background(), []string{"login"}); err != nil {
		t.Fatal(err)
	}
	if len(launchd.calls) != 0 {
		t.Errorf("calls %v", launchd.calls)
