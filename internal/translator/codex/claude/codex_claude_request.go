// Package claude translates Claude messages and Codex subscription responses.
package claude
...
import (
	...
	sigcompat "github.com/marenzo/claudex/internal/signature"
...
// ConvertRequest converts a Claude message request to a Codex subscription request.
func ConvertRequest(modelName string, inputRawJSON []byte) []byte {
	...
	if systemsResult.Exists() {
		...
		appendSystemText := func(text string) {
			if text == "" || translatorcommon.IsClaudeCodeAttributionSystemText(text) {
				return
			}
	...
	if messagesResult.IsArray() {
		...
		for i := 0; i < len(messageResults); i++ {
			...
			appendReasoningContent := func(part gjson.Result) {
				...
				signature := strings.TrimPrefix(strings.TrimSpace(part.Get("signature").String()), "gpt#")
				if !sigcompat.IsValidGPTReasoningSignature(signature) {
					return
				}
				...
				for j := 0; j < len(messageContentResults); j++ {
					...
					case "server_tool_use":
						if messageContentResult.Get("name").String() == "web_search" {
							appendTextContent("Web search query: " + messageContentResult.Get("input.query").String())
						}
					case "web_search_tool_result":
						for _, result := range messageContentResult.Get("content").Array() {
							if result.Get("type").String() == "web_search_result" {
								appendTextContent("Web search source: " + result.Get("title").String() + " " + result.Get("url").String())
							}
						}
					case "thinking":
					...
					case "image":
						...
						if imageURL := codexImageURLFromClaudeSource(messageContentResult.Get("source")); imageURL != "" {
					...
					case "tool_result":
						...
						if contentResult.IsArray() {
							...
							for k := 0; k < len(contentResults); k++ {
								...
								if toolResultContentType == "image" {
									if imageURL := codexImageURLFromClaudeSource(contentResults[k].Get("source")); imageURL != "" {
						...
						if messageContentResult.Get("is_error").Bool() {
							functionCallOutputMessage = markCodexToolResultError(functionCallOutputMessage)
						}
	...
	if format := rootResult.Get("output_config.format"); format.IsObject() && format.Get("type").String() == "json_schema" && format.Get("schema").IsObject() {
		...
		// Preserve the caller's schema when its optional or additional properties
		// are incompatible with strict output constraints.
		if strict && codexSchemaNeedsRelaxedMode(format.Get("schema")) {
			strict = false
		}
...
// Keep remote image URLs intact; the upstream handles fetching them just as it
// handles images sent directly through the Responses API.
func codexImageURLFromClaudeSource(source gjson.Result) string {
...
// Responses function_call_output has no is_error field. Preserve that distinction
// inside the tool output, keeping any image blocks intact and in order.
func markCodexToolResultError(message []byte) []byte {
	const marker = "Tool execution failed (is_error=true)."
	output := gjson.GetBytes(message, "output")
	if output.IsArray() {
		parts := make([][]byte, 0, len(output.Array())+1)
		parts = append(parts, []byte(`{"type":"input_text","text":"`+marker+`"}`))
		for _, part := range output.Array() {
			parts = append(parts, []byte(part.Raw))
		}
		message, _ = sjson.SetRawBytes(message, "output", translatorcommon.JoinRawArray(parts))
	} else {
		text := marker
		if content := output.String(); content != "" {
			text += "\n" + content
		}
		message, _ = sjson.SetBytes(message, "output", text)
	}
	return message
}
