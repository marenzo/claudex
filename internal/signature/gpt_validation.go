...
func IsValidGPTReasoningSignature(rawSignature string) bool {
	return validateGPTReasoningSignature(rawSignature) == nil
}

// validateGPTReasoningSignature validates the Fernet-like outer format used
// by GPT/Codex reasoning encrypted_content. This is only a transport-shape
// check; it does not prove decryptability.
func validateGPTReasoningSignature(rawSignature string) error {
	sig := strings.TrimSpace(rawSignature)
	if sig == "" {
		return fmt.Errorf("empty GPT reasoning signature")
	}
	if len(sig) > MaxGPTReasoningSignatureLen {
		return fmt.Errorf("GPT reasoning signature exceeds maximum length (%d bytes)", MaxGPTReasoningSignatureLen)
	}
...
// every legal character is ASCII, and a
