...
func TestHasMeaningfulCodexOutputDelta(t *testing.T) {
	...
	if !HasMeaningfulCodexOutputDelta([]byte(`{"type":"response.refusal.delta","delta":"Cannot comply"}`)) {
		t.Fatal("non-empty refusal.delta must return true")
	}
...
func TestIsCodexTerminalEmptyIncomplete(t *testing.T) {
	...
	// Case 4: Positive output tokens do not prove content was delivered.
	...
	if !IsCodexTerminalEmptyIncomplete(positiveTokens, 0, false) {
		t.Fatal("expected true for empty output despite positive output_tokens")
	}

	// Case 5: Invalid token accounting cannot turn empty output into success.
	...
	if !IsCodexTerminalEmptyIncomplete(floatTokens, 0, false) {
		t.Fatal("expected true for empty output despite fractional output_tokens")
	}
	...
	if !IsCodexTerminalEmptyIncomplete(missingTokens, 0, false) {
		t.Fatal("expected true when output_tokens is missing")
	}
	...
	if !IsCodexTerminalEmptyIncomplete(nullTokens, 0, false) {
		t.Fatal("expected true when output_tokens is null")
	}
